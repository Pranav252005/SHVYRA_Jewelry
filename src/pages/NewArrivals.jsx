...
const NewArrivals = () => {
  ...
  // Get new arrivals (for demo, using suggested products logic)
  const allProducts = getAllProducts()
  const newArrivalsProducts = [
    allProducts.find(p => p.id === 'E5'),
    allProducts.find(p => p.id === 'N2'),
    allProducts.find(p => p.id === 'E3'),
    allProducts.find(p => p.id === 'B2'),
    allProducts.find(p => p.id === 'E7'),
    allProducts.find(p => p.id === 'N1'),
    allProducts.find(p => p.id === 'E1'),
    allProducts.find(p => p.id === 'N4'),
  ].filter(Boolean)

  // Filter and sort products
  const filteredAndSortedProducts = useMemo(() => {
    let products = [...newArrivalsProducts]
    ...
    return products
  }, [newArrivalsProducts, selectedCategories, priceRange, sortBy])
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-[#E6B445] via-[#D4A574] to-[#C49A63] py-16 lg:py-20 mt-36">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-4 font-serif">
            New Arrivals
          </h1>
          <p className="text-lg text-white/90 max-w-2xl mx-auto">
            Discover our latest collection of exquisite jewelry pieces
          </p>
        </div>
      </section>
            ...
            <div className="bg-white rounded-xl shadow-md p-4 mb-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="text-gray-700">
                  <span className="font-semibold">{filteredAndSortedProducts.length}</span> New Arrivals Found
                </div>
